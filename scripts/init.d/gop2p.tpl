...
# Default-Stop:      0 1 6
# Short-Description: starts Gop2p
# Description:       starts the Gop2p
### END INIT INFO
...
im_start(){
    ...
    if [ "$isStart" == '' ];then
        ...
        cd $app_path && daemon "$app_path/gop2p service &""
        isStart=""
        while [[ "$isStart" == "" ]];
        do
            echo -e ".\c"
...
im_stop(){
	pids=`ps -ef|grep 'gop2p service' |grep -v grep|awk '{print $2}'`
    arr=($pids)
